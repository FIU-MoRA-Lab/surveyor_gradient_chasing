...
def main(file_path, path_path, water_feature, extent_filename=""):
    ...
    if extent_filename:
        extent_polygon_coordinates = np.loadtxt(
            extent_filename, delimiter=",", skiprows=1
        )
        extent_coordinates = np.vstack((extent_coordinates, extent_polygon_coordinates))
    ...
    if extent_filename:
        poly = Polygon(
            extent_polygon_coordinates[:, [1, 0]],
            closed=True,
            edgecolor="orange",
            facecolor="none",
            linewidth=2,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "file_path",
        type=str,
        help="Path to the CSV file containing water feature data.",
    )
    parser.add_argument(
        "path_path", type=str, help="Path to the CSV file containing path data."
    )
    parser.add_argument(
        "water_feature",
        type=str,
        default="ODO (%Sat)",
        help="Water feature to be plotted.",
    )
    parser.add_argument(
        "--extent_polygon",
        type=str,
        default=None,
        help="Polygon to be plotted (optional).",
    )
    ...
    main(args.file_path, args.path_path, args.water_feature, args.extent_polygon)
